...
class CursorController {
  ...
  bindEvents() {
    ...
    const interactiveElements = document.querySelectorAll(
      "a, button, .project-card, .service-card, .stat-item, .nav-menu-bubble",
    );
...
class ContactFormController {
  ...
  async handleSubmit() {
    ...
    try {
      ...
      await emailjs.send("service_qose4pc", "template_rhzmzcg", templateParams);
...
class ServicesDragController {
  ...
  bindEvents() {
    // Mouse events
    this.servicesCards.addEventListener(
      "mousedown",
  ...
  showDragCursor(e) {
    ...
    if (cursor && cursor.cursor && cursor.cursorFollower) {
      cursor.cursor.style.transition =
        "opacity 0.25s cubic-bezier(0.25, 0.46, 0.45, 0.94), transform 0.25s cubic-bezier(0.25, 0.46, 0.45, 0.94)";
      cursor.cursorFollower.style.transition =
        "opacity 0.25s cubic-bezier(0.25, 0.46, 0.45, 0.94), transform 0.25s cubic-bezier(0.25, 0.46, 0.45, 0.94)";
...
class ProjectsDragController {
  ...
  bindEvents() {
    // Mouse events
    this.projectsCards.addEventListener(
      "mousedown",
      this.handleMouseDown.bind(this),
    );
    document.addEventListener("mousemove", this.handleMouseMove.bind(this));
...
function initializeComponents() {
  // ========================
  // CURVED MARQUEE CONTROLLER
  // ========================
  class CurvedMarqueeController {
    constructor() {
      this.svg = document.getElementById("curved-marquee-svg");
      this.pathEl = document.getElementById("marquee-curve-path");
      this.textPathEl = document.getElementById("curved-marquee-textpath");
      this.units = []; // each unit = { textEl, sepEl }
      this.unitSpacing = 0;
      this.textWidth = 0;
      this.velocity = -1.2; // negative = left; raise to speed up
      this.text = "what we do";
      this.separator = "    ✦"; // the dash between repetitions
      this.ready = false;
    }

    init() {
      if (!this.svg || !this.pathEl || !this.textPathEl) return;
      document.fonts.ready.then(() => this._build());
    }
    ...
    _measure(content) {
      ...
      this.textPathEl.appendChild(span);
      const w = span.getComputedTextLength();
      this.textPathEl.removeChild(span);
      return w;
    }

    _build() {
      this.textWidth = this._measure(this.text);
      const sepWidth = this._measure(this.separator);
      this.unitSpacing = this.textWidth + sepWidth;

      const pathLen = this.pathEl.getTotalLength();
      const count = Math.ceil(pathLen / this.unitSpacing) + 4;
      const ns = "http://www.w3.org/2000/svg";

      for (let i = 0; i < count; i++) {
        const baseX = i * this.unitSpacing;
        const ns = "http://www.w3.org/2000/svg";

        // Main text — reset vertical position each iteration
        const textEl = document.createElementNS(ns, "tspan");
        textEl.textContent = this.text;
        textEl.setAttribute("x", baseX.toString());
        textEl.setAttribute("dy", i === 0 ? "0" : "28"); // compensate star shift
        textEl.setAttribute("fill", "#000000");
        textEl.setAttribute("opacity", "0.85");
        this.textPathEl.appendChild(textEl);

        // Separator star — shift UP to vertically center against 88px text
        const sepEl = document.createElementNS(ns, "tspan");
        sepEl.textContent = this.separator;
        sepEl.setAttribute("dy", "-28"); // nudge up (tweak this value)
        sepEl.setAttribute("font-size", "48");
        sepEl.setAttribute("fill", "#000000");
        sepEl.setAttribute("opacity", "1");
        this.textPathEl.appendChild(sepEl);

        this.units.push({ textEl, sepEl });
      }
    ...
    _animate() {
      ...
      const maxX = (this.units.length - 1) * this.unitSpacing;

      for (const unit of this.units) {
        let x = parseFloat(unit.textEl.getAttribute("x") || "0");
        x += this.velocity;

        // Seamless wrap: disappears left → reappears right
        if (x < -this.unitSpacing) x = maxX;
        if (x > maxX) x = -this.unitSpacing;

        unit.textEl.setAttribute("x", x.toString());
        unit.sepEl.setAttribute("x", (x + this.textWidth).toString());
      }
  ...
  // Initialize curved marquee
  const curvedMarquee = new CurvedMarqueeController();
  curvedMarquee.init();
...
function initAdvancedLottie() {
  // Attention Lottie
  const attentionContainer = document.getElementById(
    "lottie-attention-choose-us",
  );
...
class GlitterWrap {
  ...
  resetStar(s, initial = false) {
    ...
    if (c.reverse) {
      s.z = initial
        ? c.focalDepth + Math.random() * (1 - c.focalDepth)
        : c.focalDepth;
    } else {
    ...
    s.flashUntil = 0;
    s.nextFlash =
      this.elapsed + 1 + Math.random() * 4 * (1 / Math.max(0.0001, c.glitter));
  }
  ...
  drawFrame(deltaSec) {
    ...
    for (let i = 0; i < this.stars.length; i++) {
      ...
      const sizePersp = Math.min(
        2.5,
        (c.focalDepth / Math.max(s.z, 0.0001)) * 0.6,
      );
